      ...
      <GlassCard className="relative overflow-hidden text-center">
        ...
        <div className="relative space-y-4 py-6 sm:space-y-6 sm:py-8 md:py-10">
          ...
          <h1 className="font-nunito text-2xl font-bold leading-tight text-white sm:text-4xl md:text-5xl lg:text-6xl">
            <EditableBlock page="home" cmsKey="home.hero.title" type="text" value={cms.getText('home.hero.title')} as="span" />{' '}
            <EditableBlock
              page="home"
          ...
          <div className="flex flex-col items-stretch justify-center gap-3 px-2 sm:flex-row sm:items-center sm:px-0">
            <EditableBlock page="home" cmsKey="home.hero.btnServices" type="button" value={btnServices.label} href={btnServices.href} className="btn-primary w-full sm:w-auto" />
            <EditableBlock page="home" cmsKey="home.hero.btnReviews" type="button" value={btnReviews.label} href={btnReviews.href} className="btn-secondary w-full sm:w-auto" />
          </div>
        </div>
      </GlassCard>

      <CmsSlot page="home" slot="hero-after" blocks={customBlocks} />

      <div className="grid grid-cols-1 gap-3 min-[480px]:grid-cols-3 sm:gap-4">
        {[0, 1, 2].map((i) => (
          <GlassCard key={i} delay={i * 0.1} className="text-center">
            <EditableBlock page="home" cmsKey={`home.stat.${i}.value`} type="text" value={cms.getText(`home.stat.${i}.value`)} className="neon-logo text-2xl font-bold sm:text-3xl" as="p" />
            <EditableBlock page="home" cmsKey={`home.stat.${i}.label`} type="text" value={cms.getText(`home.stat.${i}.label`)} className="mt-1 text-xs text-white/50 sm:text-sm" as="p" />
          </GlassCard>
        ))}
      </div>

      <CmsSlot page="home" slot="stats-after" blocks={customBlocks} />

      <div>
        <EditableBlock page="home" cmsKey="home.services.title" type="text" value={cms.getText('home.services.title')} className="mb-3 text-lg font-bold text-white sm:mb-4 sm:text-xl" as="h2" />
        ...
        <div className="mt-4 text-center">
          <EditableBlock page="home" cmsKey="home.services.link" type="button" value={servicesLink.label} href={servicesLink.href} className="btn-secondary w-full sm:w-auto" />
        </div>
      </div>

      <CmsSlot page="home" slot="services-after" blocks={customBlocks} />

      {reviews[0] && (
        <div>
          <EditableBlock page="home" cmsKey="home.review.title" type="text" value={cms.getText('home.review.title')} className="mb-3 text-lg font-bold text-white sm:mb-4 sm:text-xl" as="h2" />
          <EditableReviewCard review={reviews[0]} compact />
        </div>
      )}

      <CmsSlot page="home" slot="review-after" blocks={customBlocks} />
      <CmsSlot page="home" slot="page-end" blocks={customBlocks} />
    </div>
  )
}
